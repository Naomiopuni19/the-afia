...
const Icons = {
  ...
  explore:   "M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z M12 8v4M12 12l3 3",
...
const css = {
  wrap: { display:'flex', height:'100vh', background:C.dark, color:C.text, fontFamily:"'DM Sans', 'Segoe UI', sans-serif", overflow:'hidden' },
  sidebar: { width:240, minWidth:240, background:'rgba(2,6,23,0.98)', borderRight:`1px solid ${C.border}`, display:'flex', flexDirection:'column', padding:'24px 16px', gap:6, zIndex:10 },
  logo: { display:'flex', alignItems:'center', gap:10, padding:'0 8px', marginBottom:28 },
  logoIcon: { width:32, height:32, background:`linear-gradient(135deg,${C.accent},${C.accent2})`, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 },
  logoText: { fontSize:18, letterSpacing:3, color:C.accent, fontWeight:300 },
  navItem: (active) => ({ display:'flex', alignItems:'center', gap:12, padding:'11px 14px', borderRadius:10, cursor:'pointer', fontSize:13, fontWeight:400, color: active ? C.accent : C.muted, background: active ? 'rgba(59,130,246,0.10)' : 'transparent', border: `1px solid ${active ? 'rgba(59,130,246,0.2)' : 'transparent'}`, transition:'all 0.2s', letterSpacing:'0.3px' }),
  main: { flex:1, overflow:'hidden', display:'flex', flexDirection:'column', position:'relative' },
  bgImg: { position:'absolute', inset:0, backgroundImage:`url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2064')`, backgroundSize:'cover', backgroundPosition:'center', opacity:0.04, pointerEvents:'none' },
  ...
  btnPrimary: { padding:'10px 22px', background:C.accent, color:'#ffffff', border:'none', borderRadius:10, fontSize:12, fontWeight:700, letterSpacing:'1.5px', textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit' },
  btnGhost: { padding:'10px 22px', background:'transparent', color:C.text, border:`1px solid ${C.border}`, borderRadius:10, fontSize:12, fontWeight:500, letterSpacing:'1px', textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit' },
  btnFull: { width:'100%', padding:'10px', borderRadius:10, border:`1px solid ${C.border}`, background:C.card2, color:C.text, fontSize:11, fontWeight:500, letterSpacing:'1.2px', textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit', marginTop:10, display:'flex', alignItems:'center', justifyContent:'center', gap:8 },
...
function Toast({ msg, type, visible }) {
  return (
    <div style={{ position:'fixed', bottom:24, right:24, zIndex:999, background:C.card2, border:`1px solid ${type==='green' ? 'rgba(16,185,129,0.4)' : type==='red' ? 'rgba(239,68,68,0.4)' : 'rgba(59,130,246,0.4)'}`, color: type==='green' ? C.green : type==='red' ? C.red : C.accent, borderRadius:12, padding:'12px 20px', fontSize:13, maxWidth:320, transform: visible ? 'translateY(0)' : 'translateY(80px)', opacity: visible ? 1 : 0, transition:'all 0.3s', pointerEvents:'none', boxShadow:'0 8px 32px rgba(0,0,0,0.5)' }}>{msg}</div>
  );
}
...
function Modal({ show, onClose, children }) {
  if (!show) return null;
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(4px)' }} onClick={onClose}>
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:20, padding:32, maxWidth:440, width:'90%', position:'relative' }} onClick={e => e.stopPropagation()}>
        <button onClick={onClose} style={{ position:'absolute', top:16, right:16, width:30, height:30, borderRadius:'50%', background:C.card2, border:`1px solid ${C.border}`, color:C.muted, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', fontSize:14 }}>✕</button>
        {children}
      </div>
    </div>
  );
...
export default function Welcome() {
  ...
  const fetchStaffMessages = async (room) => {
    if (!room) return;
    const { data } = await supabase.from('staff_chats').select('*').eq('room_number', room.toString()).eq('thread_status','open').order('created_at', { ascending:true });
    if (data) { setStaffMessages(data); if (data.length > 0 && !staffChatOpen) setStaffChatOpen(true); }
  };
  ...
  useEffect(() => { fetchMenu(); fetchRoomTypes(); }, []); // eslint-disable-line
  useEffect(() => {
    if (roomNumber && roomNumber !== "—") { fetchMyOrders(roomNumber); fetchLedger(roomNumber); fetchStaffMessages(roomNumber); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [roomNumber]);
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(`guest-room-${roomNumber}`)
      ...
      .on('postgres_changes', { event:'*', schema:'public', table:'staff_chats', filter:`room_number=eq.${roomNumber}` }, (payload) => {
        ...
        if (payload.eventType === 'INSERT' && payload.new.sender === 'staff') showToast('💬 New message from staff', 'accent');
      })
      .subscribe();
  ...
  const orderStatusLabel = (() => {
    if (!activeOrder) { const last = myOrders.find(o => o.status === 'delivered' || o.status === 'completed'); return last ? '✓ Order delivered' : 'No active orders'; }
  ...
  async function placeOrder() {
    ...
    const { data, error } = await supabase.from('orders').insert({ room_number: roomNumber, guest_name: guestName, guest_email: guestEmail, items: orderItems, total_amount: cartTotal, status:'pending', payment_status:'unpaid', notes:'' }).select().single();
    setPlacingOrder(false);
    if (error) { showToast(`Order failed: ${error.message}`,'red'); return; }
  ...
  async function sendRequest() {
    ...
    if (!booking) { showToast('No active booking found','red'); return; }
    const { error } = await supabase.from('service_requests').insert({ room_number: roomNumber, guest_name: guestName, request_type: selCat || 'General', item_name: selCat || 'General Request', notes: reqNote || null, status:'pending' });
    if (error) { showToast(`Request failed: ${error.message}`,'red'); return; }
  ...
  async function sendChat(msgOverride) {
    ...
    setChatInput('');
    const result = await processGuestMessage({ message: msg, guestName, roomNumber, booking, userId: user?.id || null, menuItems });
    setChatMessages(prev => [...prev, { role: 'concierge', text: result.text, offerHandoff: result.offerHandoff || false }]);
  }
  ...
  async function startStaffChat() {
    ...
    const { error } = await supabase.from('staff_chats').insert({ room_number: roomNumber, guest_name: guestName, user_id: user?.id || null, sender:'guest', message: opener, thread_status:'open' });
    if (error) { showToast(`Could not start chat: ${error.message}`,'red'); return; }
  ...
  async function sendToStaff() {
    ...
    if (!msg) return;
    setStaffInput('');
    await supabase.from('staff_chats').insert({ room_number: roomNumber, guest_name: guestName, user_id: user?.id || null, sender:'guest', message: msg, thread_status:'open' });
  }
  ...
  async function endStaffChat() {
    ...
    setStaffChatOpen(false); setStaffMessages([]);
    showToast('Chat closed. Thank you!', 'green');
  }

  async function requestCall() {
    if (!booking) { showToast('No active booking','red'); return; }
    const { error } = await supabase.from('call_requests').insert({ room_number: roomNumber, guest_name: guestName, user_id: user?.id || null, phone: HOTEL_PHONE, reason:'Guest requested call from AI concierge', status:'pending' });
    if (error) { showToast(`Could not request call: ${error.message}`,'red'); return; }
  ...
  async function settleAll() {
    ...
    if (!paystackKey) { showToast('Payment not configured','red'); return; }
    ...
    const startPaystack = () => {
      const handler = window.PaystackPop && window.PaystackPop.setup({
        ...
        metadata: { custom_fields: [{ display_name:"Suite", variable_name:"suite", value:roomNumber }, { display_name:"Guest", variable_name:"guest", value:guestName }]},
        callback: function(response) {
          (async () => {
            const ref = response.reference;
            await supabase.from('payments').insert({ reference:ref, room_number:roomNumber, user_id:user?.id||null, amount:totalOwed, currency:'GHS', status:'success', channel:'paystack', order_ids: myOrders.map(o => o.id), ledger_ids: ledger.map(l => l.id), verified_at: new Date().toISOString() });
  ...
  function hoursUntilCheckIn() {
    ...
    return (checkInTime - new Date()) / (1000 * 60 * 60);
  }
  ...
  function openModifyModal() {
    ...
    (async () => {
      const { data: room } = await supabase.from("rooms").select("type_id").eq("room_number", booking.room_number).single();
  ...
  async function submitModify() {
    ...
    try {
      ...
      if (modifyGuests > newType.capacity) { showToast(`${newType.name} fits ${newType.capacity} max`,"red"); setModifyBusy(false); return; }
      const { data: rooms } = await supabase.from("rooms").select("*").eq("type_id", modifyTypeId);
      const { data: conflicts } = await supabase.from("bookings").select("room_number, id").eq("status","ACTIVE").neq("id", booking.id).lt("check_in_date", co).gt("check_out_date", ci);
      ...
      if (!candidate) { showToast("No room available for those dates","red"); setModifyBusy(false); return; }
      const newTotal = Number(newType.price_per_night) * newNights;
      const oldRoom = booking.room_number;
      const { error } = await supabase.from("bookings").update({ check_in_date: ci, check_out_date: co, check_in_at: new Date(ci + "T15:00:00").toISOString(), check_out_at: new Date(co + "T12:00:00").toISOString(), room_number: candidate.room_number, nights: newNights, total_amount: newTotal, guest_count: modifyGuests, stay_duration_hours: newNights * 24 }).eq("id", booking.id);
      ...
      setShowModifyModal(false); setModifyBusy(false);
      const { data: updated } = await supabase.from("bookings").select("*").eq("id", booking.id).single();
      if (updated) setBooking(updated);
    } catch (e) { showToast(`Modify failed: ${e.message}`,"red"); setModifyBusy(false); }
  }
  ...
  async function submitCancel() {
    ...
    try {
      ...
      await supabase.from("rooms").update({ status: "VACANT" }).eq("room_number", booking.room_number);
      await supabase.from("guest_ledger").update({ status: canCancelFree ? "refunded" : "cancelled" }).eq("room_number", booking.room_number.toString()).eq("status", "pending");
      showToast(canCancelFree ? "Booking cancelled — full refund issued" : "Booking cancelled","green");
      setShowCancelModal(false); setCancelBusy(false); setBooking(null);
    } catch (e) { showToast(`Cancel failed: ${e.message}`,"red"); setCancelBusy(false); }
  }
  ...
  if (stayEnded) {
    return (
      <div style={{ minHeight:'100vh', background:'radial-gradient(circle at center, #0f172a, #020617)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'DM Sans', sans-serif", color:'#fff', padding:20 }}>
        <div style={{ textAlign:'center', maxWidth:480, padding:40, background:'rgba(15,23,42,0.9)', backdropFilter:'blur(24px)', border:'1px solid rgba(59,130,246,0.18)', borderRadius:28, boxShadow:'0 30px 60px rgba(0,0,0,0.6)' }}>
          <div style={{ fontSize:64, marginBottom:20 }}>✦</div>
          <div style={{ fontSize:10, color:C.accent, letterSpacing:4, fontWeight:700, textTransform:'uppercase', marginBottom:14 }}>The Afia</div>
          <h2 style={{ fontFamily:"Georgia, serif", fontSize:32, fontWeight:300, margin:'0 0 14px' }}>Thank you for staying with us</h2>
          <p style={{ color:'#94a3b8', fontSize:14, lineHeight:1.8, marginBottom:28 }}>Your stay has ended, {guestName}. It was an honour hosting you at The Afia. We hope to welcome you back soon.</p>
          <div style={{ padding:'14px 20px', borderRadius:12, marginBottom:28, background:'rgba(59,130,246,0.08)', border:'1px solid rgba(59,130,246,0.2)', fontSize:12, color:'#94a3b8' }}>You will be signed out automatically in a few seconds...</div>
          <div style={{ display:'flex', gap:12 }}>
            <button onClick={async () => { await supabase.auth.signOut(); navigate('/book'); }} style={{ flex:1, padding:16, borderRadius:14, background:'linear-gradient(135deg, #3b82f6, #2563eb)', border:'none', color:'#fff', fontSize:13, fontWeight:700, letterSpacing:1.5, textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit' }}>Book Your Next Stay</button>
            <button onClick={async () => { await supabase.auth.signOut(); navigate('/login'); }} style={{ flex:1, padding:16, borderRadius:14, background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', color:'#94a3b8', fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>Sign Out Now</button>
          </div>
          <div style={{ marginTop:24, fontSize:9, color:'#475569', letterSpacing:3, textTransform:'uppercase' }}>The Afia · Designed for those who notice</div>
        </div>
      </div>
    );
  }
  ...
  if (!booking) {
    return (
      <div style={{ minHeight:'100vh', width:'100%', position:'relative', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"'DM Sans', 'Segoe UI', sans-serif", background:'#020617', overflow:'hidden', padding:20 }}>
        <style>{`@keyframes nbKenBurns{0%{transform:scale(1.0) translate(0,0)}50%{transform:scale(1.08) translate(-1.2%,-1%)}100%{transform:scale(1.0) translate(0,0)}} @keyframes nbFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}} @keyframes nbCardIn{from{opacity:0;transform:translateY(28px) scale(0.97)}to{opacity:1;transform:translateY(0) scale(1)}} @keyframes nbShimmer{0%{transform:translateX(-100%)}100%{transform:translateX(200%)}} @keyframes nbGlow{0%,100%{box-shadow:0 18px 50px rgba(59,130,246,0.28)}50%{box-shadow:0 18px 60px rgba(59,130,246,0.45)}}`}</style>
        <div style={{ position:'absolute', inset:0, backgroundImage:'url("https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=2070")', backgroundSize:'cover', backgroundPosition:'center', opacity:0.5, animation:'nbKenBurns 32s ease-in-out infinite', zIndex:1 }} />
        <div style={{ position:'absolute', inset:0, zIndex:2, background:'linear-gradient(135deg, rgba(2,6,23,0.92) 0%, rgba(2,6,23,0.6) 50%, rgba(2,6,23,0.9) 100%)' }} />
        <div style={{ position:'relative', zIndex:3, width:'100%', maxWidth:460, background:'linear-gradient(135deg, rgba(15,23,42,0.88), rgba(15,23,42,0.7))', backdropFilter:'blur(28px)', border:'1px solid rgba(59,130,246,0.2)', borderRadius:28, padding:'48px 42px', textAlign:'center', boxShadow:'0 30px 70px -15px rgba(0,0,0,0.7)', animation:'nbCardIn 0.7s cubic-bezier(0.19,1,0.22,1)', overflow:'hidden' }}>
          <div style={{ width:60, height:60, margin:'0 auto 24px', borderRadius:18, background:'linear-gradient(135deg, #3b82f6, #60a5fa)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:28, color:'#fff', boxShadow:'0 10px 30px rgba(59,130,246,0.4)', animation:'nbFloat 4s ease-in-out infinite' }}>✦</div>
          <div style={{ fontSize:10, letterSpacing:4, color:'#3b82f6', textTransform:'uppercase', fontWeight:700, marginBottom:14 }}>Private Suite Access</div>
          <h1 style={{ fontFamily:"'Georgia', serif", fontSize:38, fontWeight:300, color:'#f8fafc', margin:'0 0 14px' }}>Welcome, {guestName}</h1>
          <p style={{ color:'#94a3b8', fontSize:14, lineHeight:1.7, margin:'0 auto 30px', maxWidth:360 }}>Reserve your stay to unlock your encrypted digital key, 24/7 room service, and your personal concierge.</p>
          <button onClick={() => navigate('/book')} style={{ width:'100%', padding:16, borderRadius:14, border:'none', background:'linear-gradient(135deg, #3b82f6, #2563eb)', color:'#fff', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit', animation:'nbGlow 3s ease-in-out infinite' }}>Book Your Suite →</button>
          <button onClick={handleSignOut} style={{ width:'100%', padding:13, borderRadius:14, marginTop:12, background:'transparent', border:'1px solid rgba(59,130,246,0.15)', color:'#64748b', fontWeight:600, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>Sign Out</button>
        </div>
      </div>
    );
  }
    ...
    paddingTop: isMobile ? '70px' : '28px',
         ...
         <header style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom: isMobile ? 16 : 28 }}>
            ...
            <div style={{ display:'flex', gap:10, alignItems:'center' }}>
              ...
              {isVip && <span style={{ padding:'5px 12px', background:'rgba(59,130,246,0.12)', border:`1px solid rgba(59,130,246,0.3)`, borderRadius:20, fontSize:10, letterSpacing:'2px', textTransform:'uppercase', color:accentColor, fontWeight:600 }}>✦ VIP ACCESS</span>}
            </div>
          </header>
  ...
  <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1fr', gap: isMobile ? 12 : 16, marginBottom:16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:15, fontWeight:700, color:'#fff' }}>{booking.check_out_date ? new Date(booking.check_out_date).toLocaleDateString('en-GB', { day:'numeric', month:'short' }) : '—'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize:10, color:C.muted, letterSpacing:1.5, marginBottom:4 }}>NIGHTS · GUESTS</div>
                    <div style={{ fontSize:15, fontWeight:700, color:'#fff' }}>{booking.nights || '—'} · {booking.guest_count || 1}</div>
                  </div>
                  <div>
                    <div style={{ fontSize:10, color:C.muted, letterSpacing:1.5, marginBottom:4 }}>TOTAL</div>
                    <div style={{ ...css.serif, fontSize:20, color:accentColor, lineHeight:1 }}>₵{booking.total_amount ? Number(booking.total_amount).toLocaleString() : '—'}</div>
                  </div>
                </div>
                <div style={{ display:'flex', gap:10 }}>
                  <button onClick={openModifyModal} style={{ flex:1, padding:10, borderRadius:10, background:'rgba(59,130,246,0.08)', border:`1px solid ${C.border}`, color:accentColor, fontSize:11, fontWeight:700, letterSpacing:1.2, textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit' }}>Modify Reservation</button>
                  <button onClick={() => setShowCancelModal(true)} style={{ flex:1, padding:10, borderRadius:10, background:'rgba(239,68,68,0.08)', border:'1px solid rgba(239,68,68,0.25)', color:C.red, fontSize:11, fontWeight:700, letterSpacing:1.2, textTransform:'uppercase', cursor:'pointer', fontFamily:'inherit' }}>Cancel Booking</button>
                </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr 1fr 1fr', gap: isMobile ? 10 : 12 }}>
                {[
                  ...
                  <button key={q.tab} onClick={() => setActiveTab(q.tab)} style={{ flex:1, padding:'14px 8px', borderRadius:14, background:q.color, border:`1px solid ${q.border}`, color:q.text, fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit', display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
  ...
  <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:14, marginBottom:20 }}>
                {[
                  ...
                  { icon:'🏋', name:'Fitness Center', floor:4, hours:'Open 24 hours', info:'Personal trainers available', color:'rgba(245,158,11,0.12)', border:'rgba(245,158,11,0.25)', action:'Book Trainer' },
                  { icon:'🍸', name:'Sky Bar', floor:15, hours:'5 PM – 2 AM', info:'Panoramic city views', color:'rgba(16,185,129,0.12)', border:'rgba(16,185,129,0.25)', action:'Reserve Table' },
                ].map(f => (
                  <div key={f.name} style={{ ...css.card, background:f.color, border:`1px solid ${f.border}` }}>
                    ...
                    <div style={{ fontSize:11, color:C.muted, marginBottom:14 }}>{f.info}</div>
                    <button style={{ ...css.btnFull, marginTop:0 }} onClick={() => showToast(`${f.action} request sent!`, 'accent')}>{f.action}</button>
                  </div>
                ))}
              </div>
              <div style={css.card}>
                <div style={css.cardHeader}><span style={css.cardTitle}>Day Experiences</span><span style={{ color:accentColor }}>✦</span></div>
                <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                  {[
                    ...
                    <div key={t.name} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'12px 14px', borderRadius:12, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        ...
                        <button style={{ ...css.btnFull, marginTop:0, padding:'6px 14px', width:'auto' }} onClick={() => showToast(`${t.name} booking request sent!`, 'accent')}>Book</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ ...css.card, marginTop:14 }}>
               ...
               <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : '1fr 1fr', gap:10 }}>
                  {[
                    ...
                    <button key={t.name} onClick={() => showToast(`${t.name} arranged!`, 'accent')} style={{ padding:'14px 12px', borderRadius:12, background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', color:'#fff', cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
          ...
          {activeTab === 'dining' && (
            <>
              ...
              {menuItems.length === 0 ? (
                <div style={{ ...css.card, textAlign:'center', padding:40, color:C.muted }}>The menu is being prepared. Please check back shortly.</div>
                    ...
                    {menuByCat[cat].map(item => (
                      <div key={item.id} style={css.card}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                          <div style={{ flex:1 }}>
                            {item.image_url ? <img src={item.image_url} alt={item.name} style={{ width:'100%', height:90, objectFit:'cover', borderRadius:10, marginBottom:8 }} onError={e => e.target.style.display='none'} /> : <div style={{ fontSize:22, marginBottom:8 }}>{CAT_ICON[item.category] || CAT_ICON.default}</div>}
              ...
              <div style={css.card}>
                <div style={css.cardHeader}><span style={css.cardTitle}>Your Cart</span><span style={{ fontSize:16 }}>🛒</span></div>
                {cartItems.length === 0 ? <div style={{ textAlign:'center', padding:'10px 0', fontSize:13, color:C.muted }}>Cart is empty</div> : cartItems.map(ci => (
                  <div key={ci.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid rgba(255,255,255,0.04)', fontSize:13 }}>
                    ...
                    <span style={{ display:'flex', alignItems:'center', gap:10 }}>
                      ...
                      <button onClick={() => setCartItems(prev => prev.filter(c => c.id !== ci.id))} style={{ background:'none', border:'none', color:C.red, cursor:'pointer', fontSize:14 }}>✕</button>
                    </span>
                  </div>
                ))}
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', borderTop:`1px solid ${C.border}`, paddingTop:12, marginTop:8 }}>
                  <span style={{ fontSize:13, color:C.muted }}>Total: <span style={{ ...css.serif, fontSize:18, color:C.text }}>₵{cartTotal.toFixed(2)}</span></span>
                  <button style={{ ...css.btnPrimary, opacity: placingOrder ? 0.6 : 1 }} disabled={placingOrder} onClick={placeOrder}>{placingOrder ? 'Placing...' : 'Place Order'}</button>
                </div>
              </div>
            </>
          )}
          ...
          {activeTab === 'concierge' && (
            <>
              <div style={{ marginBottom:20, display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
                <div>
                  <div style={{ fontSize:10, letterSpacing:'3px', color:accentColor, textTransform:'uppercase', fontWeight:600, marginBottom:6 }}>{staffChatOpen ? 'Live Staff Chat' : 'AI Concierge'}</div>
                  <h2 style={{ ...css.serif, fontSize:26, fontWeight:300, color:'#f8fafc' }}>{staffChatOpen ? 'Connected with our team' : 'How can I assist you?'}</h2>
                </div>
                {staffChatOpen && <button style={{ ...css.btnGhost, color:C.red, borderColor:'rgba(239,68,68,0.3)' }} onClick={endStaffChat}>End Chat</button>}
              </div>
              {!staffChatOpen ? (
                <div style={css.card}>
                    ...
                    {chatMessages.map((m, i) => (
                      <div key={i}>
                        <div style={{ display:'flex', justifyContent: m.role==='user' ? 'flex-end' : 'flex-start' }}>
                          <div style={{ maxWidth:'80%', padding:'10px 14px', fontSize:13, lineHeight:1.5, borderRadius: m.role==='user' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', background: m.role==='user' ? 'rgba(59,130,246,0.15)' : 'rgba(255,255,255,0.04)', border: `1px solid ${m.role==='user' ? 'rgba(59,130,246,0.25)' : 'rgba(255,255,255,0.06)'}` }}>{m.text}</div>
                        </div>
                        {m.offerHandoff && (
                          <div style={{ display:'flex', gap:8, marginTop:8, flexWrap:'wrap' }}>
                            <button onClick={startStaffChat} style={{ padding:'10px 16px', borderRadius:12, border:'1px solid rgba(59,130,246,0.4)', background:'rgba(59,130,246,0.10)', color:accentColor, fontWeight:700, fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:8, fontFamily:'inherit' }}><Icon d={Icons.chat} size={14} color={accentColor} /> Chat with Staff</button>
                            <button onClick={requestCall} style={{ padding:'10px 16px', borderRadius:12, border:'1px solid rgba(245,158,11,0.4)', background:'rgba(245,158,11,0.10)', color:C.gold, fontWeight:700, fontSize:12, cursor:'pointer', display:'flex', alignItems:'center', gap:8, fontFamily:'inherit' }}><Icon d={Icons.phone} size={14} color={C.gold} /> Request a Call</button>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div style={{ display:'flex', gap:10 }}>
                    <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendChat()} placeholder="Ask about hotel services..." style={{ flex:1, background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 14px', color:C.text, fontFamily:'inherit', fontSize:13, outline:'none', height:42 }} />
                ...
                <div style={css.card}>
                  <div style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:12, background:'rgba(16,185,129,0.08)', border:'1px solid rgba(16,185,129,0.2)', marginBottom:14 }}>
                    <Icon d={Icons.headset} size={16} color={C.green} />
                    <div style={{ fontSize:12, color:C.green, fontWeight:600 }}>Live · Connected to Front Desk</div>
                  </div>
                  <div ref={staffChatRef} style={{ minHeight:280, maxHeight:380, overflowY:'auto', display:'flex', flexDirection:'column', gap:10, marginBottom:14 }}>
                    {staffMessages.length === 0 ? <div style={{ textAlign:'center', padding:30, color:C.muted, fontSize:13 }}>Connecting you to a staff member...</div> : staffMessages.map(m => (
                      <div key={m.id} style={{ display:'flex', justifyContent: m.sender === 'guest' ? 'flex-end' : 'flex-start' }}>
                        <div style={{ maxWidth:'80%', padding:'10px 14px', fontSize:13, lineHeight:1.5, borderRadius: m.sender === 'guest' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', background: m.sender === 'guest' ? 'rgba(59,130,246,0.15)' : 'rgba(16,185,129,0.12)', border: `1px solid ${m.sender === 'guest' ? 'rgba(59,130,246,0.25)' : 'rgba(16,185,129,0.25)'}` }}>
                          {m.sender === 'staff' && <div style={{ fontSize:9, color:C.green, fontWeight:700, marginBottom:4, letterSpacing:1, textTransform:'uppercase' }}>Staff · Front Desk</div>}
                          {m.message}
                        </div>
                      </div>
                    ))}
                  ...
                  <div style={{ display:'flex', gap:10 }}>
                    <input value={staffInput} onChange={e => setStaffInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendToStaff()} placeholder="Type your message..." style={{ flex:1, background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 14px', color:C.text, fontFamily:'inherit', fontSize:13, outline:'none', height:42 }} />
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                  <thead>
                    <tr style={{ fontSize:10, color:C.muted, letterSpacing:'1.5px' }}>
                      {['DATE','DESCRIPTION','STATUS','AMOUNT'].map(h => <th key={h} style={{ padding:'8px 0', textAlign: h==='AMOUNT' ? 'right' : 'left', fontWeight:500 }}>{h}</th>)}
                    </tr>
                  </thead>
                  <tbody>
                    {ledger.length === 0 ? <tr><td colSpan="4" style={{ padding:'20px 0', textAlign:'center', color:C.muted }}>No outstanding charges — you're all settled.</td></tr> : ledger.map(r => (
                      <tr key={r.id} style={{ borderTop:'1px solid rgba(255,255,255,0.04)' }}>
                        ...
                        <td style={{ padding:'10px 0' }}><span style={{ padding:'3px 10px', borderRadius:20, fontSize:10, background:'rgba(245,158,11,0.08)', color:'#f59e0b', border:`1px solid rgba(245,158,11,0.2)` }}>{r.status}</span></td>
  ...
  <div style={{ display: isMobile ? 'grid' : 'flex', gridTemplateColumns: isMobile ? '1fr' : undefined, gap:12 }}>
                ...
                <button style={css.btnPrimary} onClick={async () => {
                  ...
                  const result = await sendInvoiceEmail({
                    to: guestEmail || "efya.naomi@icloud.com",
                    guestName, roomNumber,
                    suiteType: booking?.room_types?.name || "Suite",
                    checkInDate: booking?.check_in_date,
                    checkOutDate: booking?.check_out_date,
                    nights: booking?.nights,
                    charges: ledger.map(l => ({ description: l.description || l.item_name || "Charge", amount: l.amount, category: l.category || "General" })),
                  ...
                  if (result.ok) showToast("Invoice sent to your email!", "accent");
                  else showToast("Failed to send invoice", "red");
                }}>
                  📄 Send Invoice
                </button>
              </div>
               ...
               <div style={{ ...css.grid2, gridTemplateColumns: isMobile ? '1fr' : '1.5fr 1fr' }}>
                <div style={css.card}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      ...
                      <div onClick={() => { setIsDND(d => !d); showToast(isDND ? 'DND off' : 'DND on','accent'); }} style={{ width:48, height:26, borderRadius:13, border:`1px solid ${isDND ? accentColor : C.border}`, background: isDND ? 'rgba(59,130,246,0.3)' : C.card2, cursor:'pointer', position:'relative', transition:'all 0.3s' }}>
                        <div style={{ position:'absolute', top:3, left: isDND ? 27 : 3, width:18, height:18, borderRadius:'50%', background: isDND ? accentColor : C.muted, transition:'all 0.3s' }} />
                      </div>
                    </div>
                    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <span style={{ fontSize:13 }}>Lighting Mode</span>
                      <div style={{ display:'flex', gap:6 }}>
                        {['Warm','Cool','Auto'].map(m => <button key={m} style={css.chip(lightMode === m)} onClick={() => { setLightMode(m); showToast(`Lighting: ${m}`,'accent'); }}>{m}</button>)}
                      </div>
                    </div>
                  </div>
                </div>
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                  <div style={css.card}>
                    ...
                    <input type="time" value={wakeTime} onChange={e => setWakeTime(e.target.value)} style={{ width:'100%', background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 14px', color:C.text, fontFamily:'inherit', fontSize:13, outline:'none', marginBottom:10, boxSizing:'border-box' }} />
                    <button style={{ ...css.btnPrimary, width:'100%', padding:10 }} onClick={() => showToast(`Wake-up call set: ${wakeTime}`,'accent')}>Set Wake-Up Call</button>
                  </div>
                  <div style={css.card}>
      ...
      <Modal show={showKeyModal} onClose={() => setShowKeyModal(false)}>
        ...
        <p style={{ fontSize:13, color:C.muted, lineHeight:1.6, marginBottom:20 }}>Your digital key is cryptographically secured. Hold your device near the NFC reader.</p>
        <div style={{ textAlign:'center', padding:24, background:'rgba(59,130,246,0.06)', border:`1px dashed rgba(59,130,246,0.3)`, borderRadius:14, marginBottom:20 }}>
      ...
      <Modal show={showLogout} onClose={() => setShowLogout(false)}>
        ...
        <p style={{ fontSize:13, color:C.muted, lineHeight:1.6, marginBottom:20 }}>End your session? You'll need to sign in again to access your suite.</p>
        <div style={{ display:'flex', gap:10 }}>
      ...
      <Modal show={showCallModal} onClose={() => setShowCallModal(false)}>
        <div style={{ textAlign:'center' }}>
          <div style={{ width:60, height:60, borderRadius:'50%', background:'linear-gradient(135deg, #f59e0b, #fbbf24)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 18px', boxShadow:'0 10px 30px rgba(245,158,11,0.4)' }}>
            <Icon d={Icons.phone} size={26} color="#fff" />
          </div>
          <h3 style={{ ...css.serif, fontSize:24, fontWeight:300, color:'#f8fafc', margin:'0 0 8px' }}>Call Requested</h3>
          <p style={{ fontSize:13, color:C.muted, lineHeight:1.7, marginBottom:20 }}>Our team has been notified and will call you shortly. You can also reach our front desk directly:</p>
          <a href={`tel:${HOTEL_PHONE.replace(/\s/g,'')}`} style={{ display:'block', padding:'16px 20px', background:'rgba(245,158,11,0.08)', border:'1px dashed rgba(245,158,11,0.4)', borderRadius:14, textDecoration:'none', marginBottom:20 }}>
            <div style={{ fontSize:10, color:C.gold, letterSpacing:2, fontWeight:700, marginBottom:6, textTransform:'uppercase' }}>Tap to Call</div>
            <div style={{ ...css.serif, fontSize:24, color:'#fff', fontWeight:300, letterSpacing:1 }}>{HOTEL_PHONE}</div>
          </a>
      ...
      <Modal show={showModifyModal} onClose={() => setShowModifyModal(false)}>
        <h3 style={{ ...css.serif, fontSize:24, fontWeight:300, color:'#f8fafc', margin:'0 0 8px' }}>Modify Reservation</h3>
        <p style={{ fontSize:12, color:C.muted, lineHeight:1.6, marginBottom:22 }}>Pick new dates, suite type, or guest count. We'll re-check availability and reassign your room.</p>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:14 }}>
          <div>
            <div style={{ fontSize:10, letterSpacing:1.5, color:accentColor, marginBottom:6, fontWeight:700 }}>CHECK-IN</div>
            <input type="date" value={modifyCheckIn} min={new Date().toISOString().split("T")[0]} onChange={e => setModifyCheckIn(e.target.value)} style={{ width:'100%', background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 12px', color:C.text, fontSize:13, fontFamily:'inherit', outline:'none' }} />
          </div>
          <div>
            <div style={{ fontSize:10, letterSpacing:1.5, color:accentColor, marginBottom:6, fontWeight:700 }}>CHECK-OUT</div>
            <input type="date" value={modifyCheckOut} min={modifyCheckIn} onChange={e => setModifyCheckOut(e.target.value)} style={{ width:'100%', background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 12px', color:C.text, fontSize:13, fontFamily:'inherit', outline:'none' }} />
          </div>
        </div>
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:10, letterSpacing:1.5, color:accentColor, marginBottom:6, fontWeight:700 }}>SUITE TYPE</div>
          <select value={modifyTypeId || ""} onChange={e => setModifyTypeId(Number(e.target.value))} style={{ width:'100%', background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 12px', color:C.text, fontSize:13, fontFamily:'inherit', outline:'none' }}>
            <option value="" style={{ background:C.dark }}>— Select —</option>
            {roomTypes.map(t => <option key={t.id} value={t.id} style={{ background:C.dark }}>{t.name} · ₵{Number(t.price_per_night).toLocaleString()}/night · fits {t.capacity}</option>)}
          </select>
        </div>
        <div style={{ marginBottom:22 }}>
          <div style={{ fontSize:10, letterSpacing:1.5, color:accentColor, marginBottom:6, fontWeight:700 }}>GUESTS</div>
          <select value={modifyGuests} onChange={e => setModifyGuests(Number(e.target.value))} style={{ width:'100%', background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, padding:'10px 12px', color:C.text, fontSize:13, fontFamily:'inherit', outline:'none' }}>
            {[1,2,3,4,5].map(n => <option key={n} value={n} style={{ background:C.dark }}>{n} {n===1?"Guest":"Guests"}</option>)}
          </select>
        </div>
        <div style={{ display:'flex', gap:10 }}>
          <button style={{ ...css.btnGhost, flex:1 }} onClick={() => setShowModifyModal(false)}>Cancel</button>
          <button style={{ ...css.btnPrimary, flex:2, opacity: modifyBusy ? 0.6 : 1 }} disabled={modifyBusy} onClick={submitModify}>{modifyBusy ? "Updating..." : "Update Reservation"}</button>
        </div>
      </Modal>

      <Modal show={showCancelModal} onClose={() => setShowCancelModal(false)}>
        <h3 style={{ ...css.serif, fontSize:24, fontWeight:300, color:'#f8fafc', margin:'0 0 8px' }}>Cancel Reservation?</h3>
        <p style={{ fontSize:13, color:C.muted, lineHeight:1.7, marginBottom:18 }}>{canCancelFree ? "Cancelling now qualifies for a full refund. Your room will be released and your folio refunded." : "Your check-in is less than 24 hours away. Per our policy, this cancellation will not be refunded."}</p>
        <div style={{ padding:14, borderRadius:12, marginBottom:22, background: canCancelFree ? "rgba(16,185,129,0.08)" : "rgba(239,68,68,0.08)", border: `1px solid ${canCancelFree ? "rgba(16,185,129,0.25)" : "rgba(239,68,68,0.25)"}`, fontSize:12, color: canCancelFree ? C.green : C.red, fontWeight:600, textAlign:'center' }}>{canCancelFree ? "✓ Eligible for full refund" : "⚠ No refund — within 24-hour window"}</div>
        <div style={{ display:'flex', gap:10 }}>
          <button style={{ ...css.btnGhost, flex:1 }} onClick={() => setShowCancelModal(false)}>Keep Booking</button>
          <button style={{ ...css.btnPrimary, flex:2, background:C.red, opacity: cancelBusy ? 0.6 : 1 }} disabled={cancelBusy} onClick={submitCancel}>{cancelBusy ? "Cancelling..." : "Confirm Cancellation"}</button>
        </div>
      </Modal>
